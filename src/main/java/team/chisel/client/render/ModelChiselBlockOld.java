 ...
 */
@Deprecated
public class ModelChiselBlockOld implements IPerspectiveAwareModel {
    ...
    @Getter
    private @Nonnull IModelChisel model;
    private @Nonnull Overrides overrides = new Overrides();
        
    private static Cache<Pair<Item, Integer>, ModelChiselBlockOld> itemcache = CacheBuilder.newBuilder().expireAfterAccess(10, TimeUnit.SECONDS).<Pair<Item, Integer>, ModelChiselBlockOld>build();
    private static Cache<State, ModelChiselBlockOld> modelcache = CacheBuilder.newBuilder().expireAfterAccess(1, TimeUnit.MINUTES).maximumSize(5000).<State, ModelChiselBlockOld>build();
    ...
    public @Nonnull List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, long rand) {        
        ...
        ModelChiselBlockOld baked = this;
    ...
    private ModelChiselBlockOld createModel(IBlockState state, @Nonnull IModelChisel model, RenderContextList ctx) {
