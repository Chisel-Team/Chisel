...
public enum GenerationHandler implements IWorldGenerator {
    ...
    public static class WorldGenInfo {
        ...
        private Predicate<BlockState> replaceable;
    }

    private final List<Pair<WorldGenMinable, WorldGenInfo>> generators = Lists.newArrayList();

    public void addGeneration(BlockState state, WorldGenInfo info) {
    ...
    public void onLavaLakes(PopulateChunkEvent.Post event) {
        if (Configurations.basaltSpecialGen && Features.BASALT.enabled()) {
            ...
            for (BlockPos pos : BlockPos.getAllInBoxMutable(origin, origin.add(15, 11, 15))) {
                BlockState here = event.getWorld().getBlockState(pos);
                if (here.getMaterial() == Material.LAVA) {
                    BlockPos p = pos.toImmutable();
                    World w = event.getWorld();

                    int size = Configurations.basaltSideThickness;
                    for (BlockPos p2 : BlockPos.getAllInBoxMutable(p.offset(Direction.EAST, size).offset(Direction.SOUTH, size), p.offset(Direction.WEST, size).offset(Direction.NORTH, size))) {
                        replace(w, p2);
                    }

                    for (int i = 1; i <= Configurations.basaltBottomThickness; i++) {
                        replace(w, p.offset(Direction.DOWN, i));
                    }
                }
            }
        }
    ...
    private static @Nullable BlockState basaltstate;
    private static final Predicate<BlockState> replacecheck = BlockMatcher.forBlock(Blocks.STONE);
